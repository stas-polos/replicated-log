...
app.post('/messages', async (req, res) => {
  ...
  if (writeConcern > maxW) {
    logger.warn(`Write concern w=${writeConcern} exceeds available nodes (max: ${maxW}), will wait for secondaries to join`);
  }
  ...
  if (writeConcern === 1) {
    config.secondaries.forEach((secondary) => {
      axios.post(`${secondary.url}/replicate`, { id: counter, message })
  ...
  const waitForRequiredAcks = async () => {
    ...
    while (true) {
      ...
      const replications = config.secondaries.map(sec =>
        axios.post(`${sec.url}/replicate`, { id: counter, message })
          .then(() => ({ secondary: sec.name, success: true }))
          .catch(err => ({ secondary: sec.name, success: false, error: err.message }))
      );
